...
const MIGRATIONS: string[] = [
  ...
  // Migration 15 - single-session enforcement (v0.26.0+).
  ...
  // Migration 16 - audit event log (v0.34.0+). Records sensitive
  // operations for forensic post-compromise reconstruction:
  // backup downloads, file writes/deletes, RCON commands,
  // permission changes, Docker exec invocations. Different from
  // auth_events (which tracks authentication only) and
  // server_activity (which tracks user-visible server actions).
