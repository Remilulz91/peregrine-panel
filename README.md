...
> **Version 0.36.2** — second build-time patch on top of v0.36.0.
> The lockfile that shipped with v0.36.1 was generated by
> `npm install --package-lock-only` against an empty starting
> point, which silently produced a lockfile with the `resolved`
> and `integrity` fields **missing** for 174 of 212 backend
> entries. That forced `npm ci` inside the Docker build to
> re-resolve every transitive package on each build, hitting a
> separate bug in `npm@10`'s arborist that crashes on
> `@grpc/grpc-js@1.14.x` (its registry metadata has a
> `file:../proto-loader` entry that semver-compare cannot
> handle). The v0.36.2 lockfiles are regenerated incrementally on
> top of the working v0.35.2 baseline so every package has its
> `resolved` URL + `integrity` hash baked in, and `npm ci` no
> longer needs to re-resolve. See the changelog in
